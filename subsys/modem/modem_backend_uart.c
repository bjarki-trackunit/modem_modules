...
static void modem_backend_uart_receive_notify_handler(struct k_work *item)
{
	struct modem_backend_uart_work *backend_work = 
		(struct modem_backend_uart_work *)item;

	modem_pipe_notify_receive_ready(&backend_work->backend->pipe);
}
					   ...
					   const struct modem_backend_uart_config *config)
{
	...
	k_work_init(&backend->receive_ready_work.work, modem_backend_uart_receive_notify_handler);
